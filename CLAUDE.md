...
PulseGuide deploys to GitHub Pages at `hartphoenix.github.io/pulseguide/`
via `.github/workflows/deploy.yml` on every push to `main`. Build base
path is `/pulseguide/` (vite.config.ts), router basename is read from
`import.meta.env.BASE_URL`. The build copies `index.html` to `404.html`
so deep links survive refresh on GitHub Pages.

In production, maps and the manifest are fetched from
`https://hartphoenix.github.io/pulsemap/maps/`, which is the
GitHub Pages deploy of the pulsemap repo. The pulsemap
`deploy-pages.yml` workflow generates `maps/manifest.json` on every
push and ships it alongside the editor. Override with
`VITE_MAP_BASE_URL` if pointing at a fork or branch. In dev, the Vite
plugin in `vite.config.ts` serves both from `../pulsemap/maps/`.
