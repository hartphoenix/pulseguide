...
In production, maps are fetched from
`https://raw.githubusercontent.com/hartphoenix/pulsemap/main/` (manifest
at root, individual maps under `maps/`). Override with
`VITE_MAP_BASE_URL` if pointing at a fork or branch. Manifest is
generated by a workflow in the pulsemap repo on every push touching
`maps/**`. In dev, the Vite plugin in `vite.config.ts` serves both from
`../pulsemap/maps/`.
